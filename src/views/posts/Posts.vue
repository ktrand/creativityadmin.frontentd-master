      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
          ...
          <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary" @click="addNewData">
              ...
              <span class="ml-2 text-base text-primary">Добавить пост</span>
          </div>
        </div>
          ...
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <router-link :to="{name:'postContents', params:{postId:tr.id}}">
                  <feather-icon icon="CopyIcon" svgClasses="ml-2 w-5 h-5 hover:text-primary stroke-current" />
                </router-link>
              </vs-td>
...
<script>
import DataViewSidebar from '../../components/posts/PostSidebar'
...
export default {
  ...
  methods: {
    ...
    deleteData () {
      this.$store.dispatch('post/DELETE_POST', this)
    },
    editData (data) {
    ...
    deleteSelected() {
      ...
      if(this.selectedId.length > 0){
        this.$store.dispatch('trends/DELETE_SELECTED_CATEGORY', this)
      }
    },
    ...
    togglePublish(videoId){
      ...
      this.$store.dispatch('post/TOGGLE_PUBLISH', this)
    },
  },

  mounted () {
    this.isMounted = true 
    this.$store.dispatch('post/GET_POSTS', this)
  }
}
</script>
